...
//
//  2025-05-09 - JMK - Add a function to perform the special characters removal at the source so we don't need
//                     to worry about it in the bash scripts and ansible that is downstream from here.
//                     It is very important not to change the order of fields etc here as it will effect everything
//                     that is downstream.  You were WARNED.
//
...
CREATE FUNCTION CleanString(input VARCHAR(255))
RETURNS VARCHAR(255)
DETERMINISTIC
BEGIN
...
END;
";
$fun_query_res = $conn->multi_query($createFunctionQuery);
echo " fun_query_res is " . $fun_query_res;
if ($fun_query_res) {
    do {
        // Store the result set to free up the connection
        echo "About to get the results \n";
        $result = $conn->store_result();
        if ( $result ) {
            echo "About to free memory for the results \n";
            $result->free();
        }
    } while ($conn->next_result());
    echo "Function dropped and created successfully!" . "\n";
} else {
    echo "Error: " . $conn->error;
    die("Function create failed: " . $conn->error . "\n");
}
//  2025-02-28  JMK - lot  is the Latest order Transaction gained in the sub query
//
//                    os is orders in the sub-query
...
$conn->close();
//
// 2025-05-09 - JMK - Do the original work
//
...
// Check connection
if ($conn->connect_error) {
    die("Second Connection failed: " . $conn->connect_error . "\n");
}
$sql = "
